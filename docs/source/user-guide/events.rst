...
GraceDB support is LIGO/Virgo/KAGRA-specific, so it lives in the optional `asimov-gracedb` plugin rather
than asimov core. Install it first:

.. code-block:: console

   $ pip install asimov-gracedb

Once installed, you can directly download information about a trigger and create an event in the project
via asimov's generic plugin-applicator interface (``asimov apply -p``).
...
See :doc:`../gracedb` for more details.
