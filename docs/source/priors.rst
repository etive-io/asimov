...
Neither bilby nor LALInference support is bundled with asimov core (see
:doc:`pipelines`); the classes below are illustrative of the
``PriorInterface`` pattern (``asimov.priors.PriorInterface``, ``Pipeline.get_prior_interface()``
in ``asimov/pipeline.py``) that a pipeline plugin can implement, using the shape these two
pipelines' interfaces used to have when they shipped in core. They are not currently
importable from this package.

Bilby Prior Interface
~~~~~~~~~~~~~~~~~~~~~~

A ``BilbyPriorInterface`` following this pattern would look like:
...
A bilby template could then use the ``to_prior_dict_string()`` method to generate a complete prior dictionary string that can be directly inserted into the configuration file:
...
A ``LALInferencePriorInterface`` following this pattern would convert asimov priors to LALInference format:
...
A LALInference template would access these priors through the pipeline's prior interface in the same way.
